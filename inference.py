...
ENV_URL          = os.getenv("ENV_URL", "https://thrishaldevx-email-triage-env.hf.space")
...
async def run_task(task_name: str) -> None:
    ...
    # Connect to remote URL or local Docker image
    if LOCAL_IMAGE_NAME:
        # Local development: use Docker image
        env = await EmailTriageEnv.from_docker_image(
            LOCAL_IMAGE_NAME,
            env_vars={"EMAIL_TRIAGE_TASK": task_name},
        )
    else:
        # Production/validation: connect to deployed Space
